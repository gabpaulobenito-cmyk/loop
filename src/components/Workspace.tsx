...
export function Workspace() {
  ...
  const inspectorFor = (variant: 'panel' | 'drawer' | 'sheet', onDismiss?: () => void) => (
    <Inspector
      loop={selected}
      now={now}
      variant={variant}
  ...
  return (
    <div className="app" data-mode={mode}>
      ...
      {mode === 'desk' && overlay && selected && (
        <>
          <div className="scrim" onClick={() => setOverlay(false)} />
          <div className="insp-drawer" role="dialog" aria-modal="true" aria-label="Session detail">
            {inspectorFor('drawer', () => setOverlay(false))}
          </div>
        </>
      )}
      {compact && overlay && selected && (
        <div className="insp-sheet" role="dialog" aria-modal="true" aria-label="Session detail">
          {inspectorFor('sheet', () => setOverlay(false))}
        </div>
      )}
      {captureOpen && (
        <CaptureDialog
